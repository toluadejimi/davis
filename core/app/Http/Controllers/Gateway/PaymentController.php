...
class PaymentController extends Controller
{
    ...
    public function depositWalletInsert(Request $request)
    {
        ...
        $trx->status = 1;
        $trx->user_id = Auth::id();
        $trx->amount = $final_amo;
        $trx->method_code = 210;
        $trx->method_currency = "NGN";
        $trx->status = 0;
        $trx->save();
    ...
    public function depositInsert(Request $request)
    {
        ...
        if($isWallet){
            if($user->wallet->balance < $amount){
                $notify[] = ['error', "Insufficient balance, Fund Wallet"];
                return redirect('user/wallet/fund-wallet')->withNotify($notify);
                //return back()->withNotify($notify);
            }
        }
